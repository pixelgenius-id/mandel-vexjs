...
```javascript
const { Api, JsonRpc } = require('vexjs');
const { JsSignatureProvider } = require('vexjs/dist/vexjs-jssig');  // development only
...
```javascript
import { Api, JsonRpc } from 'vexjs';
import { JsSignatureProvider } from 'vexjs/dist/vexjs-jssig';  // development only
